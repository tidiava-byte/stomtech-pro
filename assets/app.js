...
(function () {
  ...
  function formatPhone(raw) {
    // Поле уже в российской маске? Тогда цифры сверх номера просто отбрасываем.
    // Судить по одному их количеству нельзя: в «+7 (930) 766-99-88» одиннадцать
    // цифр уже есть, и следующее нажатие объявило бы номер иностранным.
    var masked = /^\+7\s*\(/.test(raw);
    var d = raw.replace(/\D/g, '');
    if (!d) return '';
    // Больше одиннадцати цифр в чистом поле — это не российский номер.
    if (!masked && d.length > 11) return '+' + d;
    // Ведущая 7 или 8 — код страны или междугородний префикс, а не часть номера.
    // Отбрасываем её всегда, а не только у полного номера: иначе цифра «7»
    // из уже показанного префикса «+7» на следующем же нажатии уехала бы
    // в номер и код города сдвинулся бы на разряд.
    // Кодов городов, начинающихся с 7, в России нет, так что двусмысленности
    // не возникает: 812, 495, 930 остаются целыми.
  ...
  function initPhoneMask() {
    $$('input[type=tel]').forEach(function (el) {
      // Префикс заранее не подставляем. Пустое поле с «+7» выглядит удобным,
      // но ломает самый частый ввод: человек набирает «8 930 …», восьмёрка
      // ложится после готовой семёрки, и код города съезжает на разряд.
      // Пустое поле показывает подсказку, а «+7» дорисовывает маска
      // с первой же цифры.
