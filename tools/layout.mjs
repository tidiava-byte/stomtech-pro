...
/* Розничная продажа физическим лицам: онлайн-оплата на сайте и кассовый чек по 54-ФЗ.
   Пока false — сайт работает только с организациями и ИП, и так написано везде:
   в форме заказа, в условиях продажи и в оферте.

   Переключать в true СТРОГО одновременно с боевым запуском приёма оплаты в ЮKassa
   (инструкция — YUKASSA.md во внешней папке проекта). Обещать частному покупателю
   покупку, которую сайт не может провести, хуже, чем не обещать вовсе.

   Флаг переключает три места разом: раздел «Оплата» в условиях продажи (termsBlock),
   розничный раздел публичной оферты (директива <!-- @retail --> в tools/content/oferta.html)
   и подсказку в форме заказа. */
export const RETAIL = false;
...
export const SITE = {
  ...
  // MAX. Мессенджер не умеет ссылок по номеру телефона — это личная ссылка
  // из приложения (Профиль → поделиться). Если строку очистить, в форме
  // вместо ссылки покажется номер: по нему нас находят поиском в MAX.
  max: 'https://max.ru/u/f9LHodD0cOIuFdpd6g18Y3W7ccHuklPT-gsn9YG1PFM9CfVe0WutTvGdYWY',
...
/* Авторы материалов блога.
   В медицинской тематике и Яндекс, и Google оценивают экспертность конкретного
   человека: у подписи «экспертная группа» проверять нечего — нет ни имени, ни
   квалификации, ни страницы, где это подтверждается. Поэтому по умолчанию статьи
   подписаны основателем: практикующий врач с категорией и своей клиникой — это
   ровно тот сигнал доверия, который поисковик умеет считывать.
   `expert` оставлен для материалов, у которых личного автора действительно нет
   (технические описания производства): в front-matter статьи — "author": "expert". */
export const AUTHORS = {
  olga: {
    kind: 'Person',
    name: 'Ольга Харламова',
    role: 'Врач-ортодонт, основатель STOMTECH PRO',
    jobTitle: 'Врач-ортодонт, врач высшей категории',
    initials: 'ОХ',
    photo: 'assets/img/photo/olga-harlamova.webp',
    page: 'avtor-olga-harlamova',
    bio: 'Врач-ортодонт высшей категории, доктор в третьем поколении, более 12 лет клинической практики. ' +
         'Собственник стоматологической клиники «Стомтек» и основатель STOMTECH PRO: линейка порошков ' +
         'выросла из её собственного приёма, где не хватало расходника с предсказуемым качеством.',
    knowsAbout: [
...
/* Автор по умолчанию: подставляется всюду, где в front-matter не указано иное. */
export const DEFAULT_AUTHOR = 'olga';
...
/* ---------- <head> ----------
   ...
   • `favicon.ico` в корне сайта — единственный адрес, который браузеры, Телеграм
     и поисковые роботы запрашивают сами, не читая разметку. Пока его не было,
     `/favicon.ico` отдавал 404, и в выдаче с историей в кэше держалась старая иконка.
     Файл собран из трёх размеров (16, 32, 48) — `favicon.ico` в корне site/.
   • SVG — то, что реально показывает современный браузер: он резкий на любом экране.
...
export function termsBlock() {
  return `<div class="terms">
      ...
      <ul class="check">
        ...
        <li>Цены указаны на страницах каталога в рублях и едины для всех покупателей. Продавец освобождён от НДС как участник проекта «Сколково» — в счёте и документах «Без налога (НДС)».</li>
      </ul>
      ...
      <h4>Доставка</h4>
      <ul class="check">
        <li>По всей России и в страны ближнего зарубежья, отгрузка с производства в Курске.</li>
        ${RETAIL
          ? `<li>СДЭК до пункта выдачи или курьером по адресу. Стоимость рассчитывается при оформлении заказа по тарифам СДЭК и оплачивается покупателем.</li>`
          : `<li>Способ, стоимость и срок доставки менеджер называет при подтверждении заявки — до выставления счёта. Возможен самовывоз с производства по согласованию.</li>`}
        <li>Отгрузка — в течение 10 дней с момента оплаты.</li>
      </ul>

      <h4>Возврат</h4>
      <ul class="check">
        <li>Брак, недостача или пересорт — заменяем товар либо возвращаем оплату. Зафиксируйте расхождение при приёмке и сообщите нам.</li>
        ${RETAIL
          ? `<li>Частный покупатель вправе отказаться от заказа в любое время до получения и в течение 7 дней после (ст. 26.1 закона «О защите прав потребителей»), если сохранены товарный вид, потребительские свойства и упаковка.</li>
        <li>Деньги возвращаются тем же способом, которым была произведена оплата, в течение 10 дней с момента получения заявления. Расходы на обратную пересылку несёт покупатель.</li>`
          : `<li>Возврат качественного товара, поставленного в согласованном ассортименте и количестве, регулируется договором и нормами Гражданского кодекса о поставке — порядок согласовывается индивидуально.</li>`}
      </ul>

      <p class="note mt-s">Полные условия сделки — в <a href="oferta.html" class="tlink">публичной оферте</a>.</p>
    </div>`;
}
